...
const objLat = (obj) => {
    ...
    let upperFirstName = obj.firstName.charAt(0).toUpperCase() + obj.firstName.slice(1);
  
    let upperLastName = obj.lastName.charAt(0).toUpperCase() + obj.lastName.slice(1);
    
    let fullName=upperFirstName +" "+ upperLastName;
...
//  Required:
//
//  Rekey is receiving info about applicants for his startup company (as ar+ray of objects), containing first name,
...
const cvFormatter = (arr) => {
     let cvsFormatt=[];
     let filterNullValues=[];
        for(let i=0;i<arr.length;i++)
        {   
            if(arr[i].yearsOfExperience >1)
            { if(arr[i].lastName===null)
                {   delete arr[i].lastName;
                    cvsFormatt.push({fullName:`${arr[i].firstName}`,tech:`${arr[i].tech}`});   
                }
                else
                {
                    cvsFormatt.push({fullName:`${arr[i].firstName} ${arr[i].lastName}`,tech:`${arr[i].tech}`});
                }
            }  
